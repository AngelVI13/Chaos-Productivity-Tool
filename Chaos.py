...
class TaskHistoryPopup(QtGui.QWidget):
	...
	def recover_task(self):
		...
		self.completed_tasks.remove(task)  # removes the task from the completed list
		self.emit(QtCore.SIGNAL('recover_task(QString)'), task)
...
class TimerApp(QtGui.QMainWindow, design.Ui_MainWindow):
	...
	task_list = []  # emty list of current tasks
	completed_tasks = []  # empty list of completed tasks
	...
	def add_task(self):
		task, ok = QtGui.QInputDialog.getText(self, 'Add Task', 'Enter task')
		
		if ok:
			if task != '':
				self.task_list.append(task)  # add task to task list
				taskWidget = QtGui.QCheckBox(task)
				taskWidget.stateChanged.connect(lambda x: self.complete_task(taskWidget))
				self.taskWidgetsVLayout.addWidget(taskWidget)
			else:
				QtGui.QMessageBox.warning(self, 'Error', 'Task field left blank!', QtGui.QMessageBox.Ok)


	def load_saved_tasks(self, task):
		self.task_list.append(task)  # adds task to current tasks
		taskWidget = QtGui.QCheckBox(task)
		taskWidget.stateChanged.connect(lambda x: self.complete_task(taskWidget))
		self.taskWidgetsVLayout.addWidget(taskWidget)
	...
	def complete_task(self, task):
		self.task_list.remove(task.text())  # remove from current tasks
		self.completed_tasks.append(task.text())  # adds it to the completed task list
	...
	def show_task_history(self):
		...
		self.connect(self.historyWindow, QtCore.SIGNAL('recover_task(QString)'), self.recover_task)
		self.historyWindow.show()


	def recover_task(self, task):
		self.task_list.append(task)  # add task to current task list
		taskWidget = QtGui.QCheckBox(task)
		taskWidget.stateChanged.connect(lambda x: self.complete_task(taskWidget))
		self.taskWidgetsVLayout.addWidget(taskWidget)
